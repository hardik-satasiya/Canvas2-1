...
final class Canvas2Tests: XCTestCase {
    ...
    func testPathMethod() {
        do {
            let testMethods: [Path.Method] = [.stroke(2), .dash(1, 2, [3, 4]), .fill]
            for method in testMethods {
                let data = try JSONEncoder().encode(method)
                let result = try JSONDecoder().decode(Path.Method.self, from: data)
    ...
    func testPath() {
        ...
        let path = Path(path: cgPath, method: .stroke(1), color: .blue)
        do {
            let data = try JSONEncoder().encode(path)
            let result = try JSONDecoder().decode(Path.self, from: data)
    ...
    func testText() {
        ...
        let text = Text(string: .init(string: str, attributes: attrs), at: .zero, rotation: .pi)
        do {
            let data = try JSONEncoder().encode(text)
            let result = try JSONDecoder().decode(Text.self, from: data)
